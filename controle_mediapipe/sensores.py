...
class FootButtonController:
    """
    Lê a webcam, detecta pés com MediaPipe Pose e aciona:
      - jump_edge: True apenas no frame em que algum pé entra na ROI de pulo
      - crouching_hold: True enquanto algum pé está na ROI de agachar
    Mostra uma janela de depuração com as ROIs e pontos do pé.
    """
    def __init__(
        self,
        cam_index: int = 0,
        ...
        # estados públicos
        self._jump_edge = False         # borda: true só no frame do "pisou"
        self._crouching_hold = False    # nível: true enquanto pé na ROI

        # estados internos p/ debounce
        self._in_jump_area_prev = False
        self._stop = False
        self._thread: Optional[threading.Thread] = None

        # para suavizar (anti ruído)
    ...
    def _run(self):
        ...
        with mp_pose.Pose(
            ...
            while not self._stop:
                ...
                # suavização simples (exponencial)
                ...
                # lógica das ROIs
                any_in_jump = self._foot_in_roi(self._left_px, self._left_py, self.jump_roi) or \
                              self._foot_in_roi(self._right_px, self._right_py, self.jump_roi)
                any_in_crouch = self._foot_in_roi(self._left_px, self._left_py, self.crouch_roi) or \
                                self._foot_in_roi(self._right_px, self._right_py, self.crouch_roi)

                # borda de entrada no botão de PULO
                self._jump_edge = (not self._in_jump_area_prev) and any_in_jump
                self._in_jump_area_prev = any_in_jump

                # nível mantido para AGACHAR
                ...
                if self.show_debug:
                    dbg = frame.copy()
                    # ROIs
                    cv2.rectangle(dbg, (self.jump_roi.x, self.jump_roi.y),
                                  (self.jump_roi.x + self.jump_roi.w, self.jump_roi.y + self.jump_roi.h),
                                  (0, 200, 255), 2)
                    cv2.putText(dbg, "JUMP", (self.jump_roi.x+5, self.jump_roi.y-8),
                                cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 200, 255), 2)

                    cv2.rectangle(dbg, (self.crouch_roi.x, self.crouch_roi.y),
                                  (self.crouch_roi.x + self.crouch_roi.w, self.crouch_roi.y + self.crouch_roi.h),
                                  (255, 150, 0), 2)
                    cv2.putText(dbg, "CROUCH", (self.crouch_roi.x+5, self.crouch_roi.y-8),
                    ...
                    # estados
                    cv2.putText(dbg, f"jump_edge: {self._jump_edge}", (10, 20),
                                cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)
                    cv2.putText(dbg, f"crouching: {self._crouching_hold}", (10, 45),
                                cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)
    ...
    def _extract_feet_pixels(self, res, w: int, h: int) -> Tuple[Optional[int], Optional[int], Optional[int], Optional[int]]:
        ...
        def to_px(idx_a, idx_b):
            ...
            # centro pé simples: média calcanhar/ponta
